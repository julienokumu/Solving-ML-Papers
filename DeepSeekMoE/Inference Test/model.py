...
class RoPE(nn.Module):
    def __init__(self, head_dim: int, theta: torch.Tensor, max_seq_len: int = 128):
        super().__init__()
...
class MoE(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.n_shared_expers = args.n_shared_experts
...
class MHA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        ...
        output = attn_output.transpose(1, 2).contiguous().view(batch, seq_len, -1)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        ...
        attn_output = self.attn(x, positions) + x_norm
