...
class GQA(nn.Module):
    def __init__(self, d_model: int, n_heads: int, n_kv_heads: int):
        ...
        assert d_model == self.head_dim * n_heads, " d_model must be divisible by n_heads"
...
class DeepSeek67B(nn.Module):
    def __init__(self, d_model: int = 8192, n_heads: int = 64, n_kv_heads: int = 8, n_layers: int = 95, context_length: int = 4096, vocab_size: int = 102400):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        print("===DeepSeek67B Test===")
        print(f"input shape: {x.shape}, output shape: {logits.shape}")
        return logits

try:
    print("starting model test....")
    model = DeepSeek67B(d_model=128, n_heads=8, n_kv_heads=2, n_layers=2, vocab_size=1000)
    ...
    x = x.to(device)
    print(f"model and input moved to device: {device}")
    model.eval()
    with torch.no_grad():
        logits = model(x)
