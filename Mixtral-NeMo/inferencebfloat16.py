...
mask = jnp.tril(jnp.ones((1, 1, 128, 128), dtype=jnp.bfloat16))
theta = 1.0 / (args.base ** (jnp.arange(0, args.head_dim, 2, dtype=jnp.bfloat16) / args.head_dim))
...
class RMSNorm(nn.Module):
    ...
    def setup(self):
        self.gamma = self.param('gamma', lambda rng, shape: nn.initializers.ones(rng, shape, dtype=jnp.bfloat16), (self.dim,))
...
class SwiGLUFFN(nn.Module):
    ...
    def setup(self):
        self.linear1 = nn.Dense(self.hidden_dim, dtype=jnp.bfloat16)
        self.gate = nn.Dense(self.hidden_dim, dtype=jnp.bfloat16)
        self.linear2 = nn.Dense(self.dim, dtype=jnp.bfloat16)
        self.beta = self.param('beta', lambda rng, shape: nn.initializers.ones(rng, shape, dtype=jnp.bfloat16), (1,))
...
class RoPE(nn.Module):
    ...
    def __call__(self, x, seq_len=None, theta=theta):
        ...
        positions = jnp.arange(seq_len, dtype=jnp.bfloat16)
        freqs = jnp.outer(positions, theta).astype(jnp.bfloat16)
...
class MHA(nn.Module):
    ...
    def setup(self):
        self.q_proj = nn.Dense(self.n_heads * self.head_dim, use_bias=False, dtype=jnp.bfloat16)
        self.k_proj = nn.Dense(self.n_kv_heads * self.head_dim, use_bias=False, dtype=jnp.bfloat16)
        self.v_proj = nn.Dense(self.n_kv_heads * self.head_dim, use_bias=False, dtype=jnp.bfloat16)
        self.out_proj = nn.Dense(self.dim, use_bias=False, dtype=jnp.bfloat16)
...
class MixtralNeMo(nn.Module):
    ...
    def setup(self):
        self.emb = nn.Embed(self.vocab_size, self.dim, dtype=jnp.bfloat16)
        self.layers = [Block(
            self.dim,
            self.n_heads,
            self.n_kv_heads,
        ...
        self.lm_head = nn.Dense(self.vocab_size, use_bias=False, dtype=jnp.bfloat16)
    
    def __call__(self, input_ids, mask=None):
        batch, seq_len = input_ids.shape

        x = self.emb(input_ids).astype(jnp.bfloat16)
...
def inference():
    ...
    print(f"jax using {os.environ.get('JAX_NUM_THREADS', 'not set')} threads")
